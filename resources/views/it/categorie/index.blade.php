...
<div class="d-flex flex-column flex-column-fluid">
    ...
    <div class="card-title">
        ...
        <form method="GET" action="{{ route('categorie.show.index') }}" class="d-flex align-items-center position-relative my-1">
            <i class="ki-duotone ki-magnifier fs-3 position-absolute ms-5"><span class="path1"></span><span class="path2"></span></i>
            <input 
                type="text" 
                name="search" 
                value="{{ $search ?? '' }}" 
                class="form-control form-control border border-gray-300 w-250px ps-13" 
                placeholder="Rechercher une catégorie" />
            <button type="submit" class="btn btn-primary ms-3">Rechercher</button>
        </form>
    </div>
    <div class="card-toolbar">
        <a href="{{ route('categorie.create.show') }}">
            <button type="button" class="btn btn-light-primary">
                ...
                <div class="card-body pt-0">
                    ...
                    <div id="kt_permissions_table_wrapper" class="dt-container dt-bootstrap5 dt-empty-footer">
                            ...
                            <<table class="table align-middle table-row-bordered fs-6 gy-5 mb-0 dataTable" id="kt_permissions_table" style="width: 100%;">
                                ...
                                <tbody class="fw-semibold text-gray-600 border-top">
                                    ...
                                    <tr>
                                        <td>{{ $categorie->nom }}</td>
                                        <td>
                                            @if($categorie->created_at) {{ $categorie->created_at }} @else - @endif
                                        </td>
                                        <td class="text-end">
                                            <a href="{{ route("categorie.subcategorie.show", ['id_categorie'=> $categorie->id]) }}">
                                                <button class="btn btn-icon btn-light-primary w-30px h-30px" title="Sous catégories">
                                                    <i class="fa-solid fa-layer-group"></i>
                                                </button>
                                            </a>
                                            <a href="{{ route('it.categorie.show', ['id' => $categorie->id]) }}">
                                                <button class="btn btn-icon btn-light-primary w-30px h-30px" data-kt-permissions-table-filter="delete_row">
                                                    <i class="fa-solid fa-eye"></i>
                                                </button>
                                            </a>
                                            <a href="{{ route('categorie.supprimer', ['id' => $categorie->id]) }}" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cette Catégorie ?')">
                        ...
                        <div id="" class="row">
                            {!! $categories->links('vendor.pagination.kevinPagination') !!}
                        </div>
                    </div>
                    <!--end::Table-->
                </div>
