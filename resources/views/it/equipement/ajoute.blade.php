    ...
    <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
        <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex flex-stack">
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                ...
                <span class="fs-6 text-muted">Renseigne la catégorie, les attributs dynamiques et l'attribution en quelques clics.</span>
            </div>
        </div>
    </div>
        ...
        <div id="kt_app_content_container" class="app-container container-fluid">
            <div class="d-flex flex-column flex-xl-row">
                <div class="card bg-body me-xl-9 mb-9 mb-xl-0 flex-grow-1">
                    <div class="card-body">
                        <form method="POST" action="{{ route('equipement.create') }}" class="form" data-dynamic-form data-assign-form
                              data-requires-comment="false"
                              ...
                              data-existing-attributes='@json([])'
                              data-attribute-errors='@json($attributeErrors)'
                              data-initial-type="{{ old('type_attribution', 'service') }}"
                              data-initial-action="{{ old('id_action') }}"
                              data-initial-target="{{ old('id_service') ?? old('id_utilisateur') ?? '' }}">
                            @csrf

                            <div class="mb-7">
                                <p class="fw-semibold fs-4 text-gray-600 mb-2">
                                    Complète le formulaire, le module dynamique s'occupe d'afficher les bons champs selon la sous-catégorie.
                                </p>
                            </div>
                                ...
                                <div class="col-md-6 fv-row">
                                    ...
                                    <select class="form-control border border-gray-300" name="id_emplacement">
                                        ...
                                        @foreach ($emplacements as $emplacement)
                                            <option value="{{ $emplacement->id }}" @selected(old('id_emplacement') == $emplacement->id)>{{ $emplacement->nom }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-6 fv-row">
                                    ...
                                    <select class="form-control border border-gray-300" name="id_action" data-action-select>
                                        ...
                                        @foreach ($actions as $action)
                                            <option value="{{ $action->id }}" @selected(old('id_action') == $action->id)>{{ $action->nom }}</option>
                                        @endforeach
                                    </select>
                                    @error('id_action')
                                        <div class="text-danger small mt-2">{{ $message }}</div>
                            ...
                            <div class="row mb-5">
                                <div class="col-md-6 fv-row block">
                                    ...
                                    <div class="d-flex flex-wrap gap-4 mt-3">
                                        @php $type = old('type_attribution', 'service'); @endphp
                            ...
                            <div class="row mb-5" data-attribution-section="service" @if($type !== 'service') hidden @endif>
                                <div class="col-md-6 fv-row position-relative" data-autocomplete>
                                    ...
                                    <input type="hidden" name="id_service" value="{{ $selectedServiceId }}" data-autocomplete-hidden>
                            ...
                            <div class="row mb-5" data-attribution-section="utilisateur" @if($type !== 'utilisateur') hidden @endif>
                                <div class="col-md-6 fv-row position-relative" data-autocomplete>
                                    ...
                                    <input type="hidden" name="id_utilisateur" value="{{ $selectedUserId }}" data-autocomplete-hidden>
                            ...
                            <div class="row mb-5">
                                <div class="col-md-6 fv-row">
                                    ...
                                    <input type="text" class="form-control border border-gray-300" name="sn" value="{{ old('sn') }}" />
                                    @error('sn')
                                        <div class="text-danger small mt-2">{{ $message }}</div>
                                    @enderror
                                </div>
                                <div class="col-md-6 fv-row">
                                    ...
                                    <input type="text" class="form-control border border-gray-300" name="hostname" value="{{ old('hostname') }}" />
                                    @error('hostname')
                                        <div class="text-danger small mt-2">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="row mb-5">
                                <div class="col-md-6 fv-row">
                                    ...
                                    <input type="number" step="0.01" class="form-control border border-gray-300" name="prix" value="{{ old('prix') }}" />
                                    @error('prix')
                                        <div class="text-danger small mt-2">{{ $message }}</div>
                                    @enderror
                                </div>
                                <div class="col-md-6 fv-row">
                                    ...
                                    <input type="text" class="form-control border border-gray-300" name="description" value="{{ old('description') }}" />
                                    @error('description')
                                        <div class="text-danger small mt-2">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="row mb-5">
                                <div class="col-md-6 fv-row">
                                    ...
                                    <input type="date" class="form-control border border-gray-300" name="date_achat" value="{{ old('date_achat') }}" />
                                    @error('date_achat')
                                        <div class="text-danger small mt-2">{{ $message }}</div>
                                    @enderror
                                </div>
                                <div class="col-md-6 fv-row">
                                    ...
                                    <input type="date" class="form-control border border-gray-300" name="date_obsolescence" value="{{ old('date_obsolescence') }}" />
                            ...
                            <div class="row mb-5">
                                <div class="col-md-6 fv-row block">
                                    ...
                                    <div class="d-flex gap-4 mt-3">
                                        @php $reseau = old('equipement_reseau', 'non'); @endphp
                                        <label class="form-check form-check-custom form-check-solid align-items-center gap-2">
                            ...
                            <div class="border rounded p-5 mb-5" data-network-fields @if($reseau !== 'oui') hidden @endif>
                                    ...
                                    <div class="col-md-6 fv-row">
                                        ...
                                        <select class="form-control border border-gray-300" name="id_vlan">
                                            ...
                                            @foreach ($vlans as $vlan)
                                                <option value="{{ $vlan->id }}" @selected(old('id_vlan') == $vlan->id)>{{ $vlan->nom }}</option>
                                            @endforeach
                                        </select>
                                        @error('id_vlan')
                                            <div class="text-danger small mt-2">{{ $message }}</div>
                                ...
                                <div class="row mb-5">
                                    <div class="col-md-6 fv-row">
                                        ...
                                        <input type="text" class="form-control border border-gray-300" name="adresse_ip" value="{{ old('adresse_ip') }}" />
                                    </div>
                                    <div class="col-md-6 fv-row">
                                        <label class="fs-5 fw-semibold mb-2">Adresse MAC</label>
                                        <input type="text" class="form-control border border-gray-300" name="adresse_mac" value="{{ old('adresse_mac') }}" />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mb-8" data-attribute-container>
                                <div class="alert alert-secondary mb-0">
                                    Sélectionne une sous-catégorie pour afficher les attributs métiers associés.
                                </div>
                            </div>

                            <div class="row mb-5">
                                <div class="col-md-8 fv-row">
                                    <label class="fs-5 fw-semibold mb-2">Commentaire</label>
                                    <textarea class="form-control border border-gray-300" name="commentaire" rows="3" data-comment-field placeholder="Ajoute du contexte si besoin.">{{ old('commentaire') }}</textarea>
                                    @error('commentaire')
                                        <div class="text-danger small mt-2">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <button type="submit" class="btn btn-primary">
                                <span class="indicator-label">Créer l'équipement</span>
                            </button>
                        </form>
                    </div>
                </div>
